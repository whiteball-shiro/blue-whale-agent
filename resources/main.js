...
async function collectMcpTools(force) {
  ...
  if (loadConfig().chatMcp === true) {
    ...
    const builtinFileTools = [
      { name: 'whale_list_dir', desc: '列出目录下的文件和子目录。用户说“桌面”时 path 填 [桌面]（会自动解析成真实桌面）。', tool: 'list_dir', schema: { type: 'object', properties: { path: { type: 'string', description: '目录路径。桌面可用 [桌面]' } }, required: ['path'] } },
