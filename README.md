...
**一行概括**：可爱桌宠 + 通用 LLM 对话 + 本地模型分流（省额度）+ 本地生图 + 文件工具。**没有显卡也能用**（余额 + 云端聊天），有显卡再解锁本地分流/生图。

---

## ✨ 功能

**基础版（开箱即用，只需填一个 API Key）**

- 透明置顶、Q 弹的小鲸鱼桌面宠物，实时显示 API 余额
...
> **没有显卡也能用**：只开余额 + 云端聊天即可。本地分流/生图是可选进阶功能。
...
核心桌宠（Electron）可跨平台；`chat-workspace/` 里的本地脚本（PowerShell + GPU）目前为 Windows 专属可选模块，Mac/Linux 上可直接用基础版。
